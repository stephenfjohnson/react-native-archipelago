...
export default function MapScreen({
  ...
  const POI_INDOOR_ONLY = getSetting("POI_INDOOR_ONLY", "boolean");
  const poiMode: PoiMode = POI_LOCATIONS
    ? POI_INDOOR_ONLY
      ? "indoor"
      : "pois"
    : "roads";
  ...
  const loadOrFetchCandidates = async (
    ...
    // The candidate cache is keyed by POI mode, so toggling the point of
    // interest settings mid-session fetches a matching candidate set instead
    // of silently reusing roads-only nodes (or vice versa).
    const cacheKey =
      sessionName + "_candidates" + (poiMode === "roads" ? "" : "_" + poiMode);
    ...
    if (fetched && fetched.length > 0 && sessionName && sessionName !== "") {
      await save(fetched, cacheKey, STORAGE_TYPES.OBJECT);
    }
    return fetched;
  };
  ...
  return (
    <View style={mapStyles.container}>
      ...
      >
        ...
        />
        {USE_HOME_LOCATION && (
          <Marker lngLat={[HOME_LOCATION.longitude, HOME_LOCATION.latitude]}>
